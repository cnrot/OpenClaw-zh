...
<button class="panel-tab active" data-tab="help">帮助文档</button>
    ...
    if (!data || !data.commits || data.commits.length === 0) {
      container.innerHTML = `
        <div class="changelog-empty">
          <p>暂无更新日志</p>
          <p class="changelog-hint">请访问 <a href="https://github.com/cnrot/OpenClaw-zh" target="_blank">GitHub 仓库</a> 查看完整提交历史</p>
        </div>
      `;
      return;
    }
  ...
  async function executeCommand(action) {
    ...
    switch (action) {
      ...
      case 'check-update':
        try {
          const res = await fetch('https://registry.npmjs.org/@coryrowe/openclaw-zh/latest');
